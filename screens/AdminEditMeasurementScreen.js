...
const AdminEditMeasurementScreen = ({ navigation, route }) => {
  const { measurementId, measurement: initialMeasurement } = route.params;
  const [saving, setSaving] = useState(false);
  const [notes, setNotes] = useState(initialMeasurement?.notes || '');
  
  const [sections, setSections] = useState([]);

  useEffect(() => {
    // Initialize sections from measurement data
    if (initialMeasurement?.sections) {
      const formattedSections = initialMeasurement.sections.map((section, index) => ({
        id: index + 1,
        bodyPart: section.sectionName || '',
        measurementUnit: 'cm',
        showUnitDropdown: false,
        fields: section.measurements?.map((m, fieldIndex) => ({
          id: fieldIndex + 1,
          bodyPartTitle: m.bodyPartName || '',
          sizes: m.size?.toString() || '',
          sizeError: ''
      ...
      setSections([{
        ...
        measurementUnit: 'cm',
        showUnitDropdown: false,
        fields: [{ id: 1, bodyPartTitle: '', sizes: '', sizeError: '' }]
      }]);
    }
  }, [initialMeasurement]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await ApiService.updateAdminMeasurement(measurementId, {
      ...
      if (response.success) {
        ...
        Alert.alert('Error', response.message || 'Failed to update measurement');
      }
    } catch (error) {
      ...
      <View style={styles.actionButtons}>
        ...
        >
          {saving ? (
            ...
            <Text style={styles.saveButtonText}>Update Measurement</Text>
          )}
        </TouchableOpacity>
      </View>
    </View>
...
export default AdminEditMeasurementScreen;