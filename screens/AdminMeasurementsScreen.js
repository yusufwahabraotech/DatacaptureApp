...
const AdminMeasurementsScreen = ({ navigation }) => {
  ...
  const handleExportAll = async () => {
    ...
    // Debug: Log the first measurement to see the data structure
    console.log('=== MEASUREMENT DATA STRUCTURE ===');
    ...
    try {
      const htmlContent = `
        ...
        <head>
          <meta charset="utf-8">
          <title>All Measurements Report</title>
          <style>
        ...
        <body>
          <div class="header">
            <div class="title">All Measurements Report</div>
          ...
          <table>
            ...
            <tbody>
              ${measurements.map(measurement => {
                ...
                // Debug each measurement
                console.log('Processing measurement:', measurement._id || measurement.id);
                console.log('Available keys:', Object.keys(measurement));
                
                // Try different possible data structures
                if (measurement.sections && Array.isArray(measurement.sections)) {
                  console.log('Found sections array:', measurement.sections);
                  ...
                  console.log('Found data object with entries:', entries);
                  if (entries.length > 0) {
                    measurementText = entries.map(([key, value]) => `${key}: ${value}cm`).join(', ');
                  }
                } else {
                  console.log('No measurement data found in expected locations');
                }
                
                console.log('Final measurement text:', measurementText);
      ...
      if (await Sharing.isAvailableAsync()) {
        await Sharing.shareAsync(uri, {
          ...
          dialogTitle: 'Export All Measurements'
        });
      }
    } catch (error) {
